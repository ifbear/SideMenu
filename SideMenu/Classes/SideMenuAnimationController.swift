...
final class SideMenuAnimationController: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    ///   - config: Model
    ///   - leftSide: Bool
    ///   - delegate: SideMenuAnimationControllerDelegate
    ...
    internal func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        guard let presentedViewController = transitionContext.presentedViewController,
    ...
    /// animationEnded
    /// - Parameter transitionCompleted: Bool
    internal func animationEnded(_ transitionCompleted: Bool) {
        guard let presentedViewController = presentedViewController else { return }
        if presentedViewController.side.isHidden == true {
    ...
    internal func transition(presenting: Bool, animated: Bool = true, interactive: Bool = false, alongsideTransition: (() -> Void)? = nil, complete: Bool = true, completion: ((Bool) -> Void)? = nil) {
        ...
        transition(presenting: presenting, animated: animated, interactive: interactive, animations: { [weak self] in
...
extension SideMenuAnimationController {
    ...
    /// prepare presenting
    /// - Parameter presenting: presentingViewController
    private func prepare(presenting: Bool) {
        guard presenting == true, let presentingViewController = presentingViewController, let presentedViewController = presentedViewController else { return }
    ...
    private func finish(presenting: Bool, completed: Bool) {
        guard presenting != completed, let presentingViewController = self.presentingViewController else { return }
    ...
    private func transition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        transition(presenting: transitionContext.isPresenting, animated: transitionContext.isAnimated, interactive: transitionContext.isInteractive, animations: { [weak self] in
            guard let self = self else { return }
    ...
    /// transition
    private func transition(presenting: Bool, animated: Bool = true, interactive: Bool = false,
